...
exports.forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  const mensaje = `Hola!\n\nEl link para restablecer la contraseña es : \n\n${resetUrl}
  \n\nSi no solicitaste restablecer la contraseña, por favor comunicate con soporte.
  \n\nAtt:\Ezenty - Fragancias que inspiran`;

  try {
    await sendEmail({
      email: user.email,
      subject: "Restablecer contraseña - Ezenty Fragrancias que inspiran",
      mensaje,
    });
    res.status(200).json({
      success: true,
...
exports.resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHandler(
        "El token es invalido, si aun necesita restablecer la contraseña solicite uno nuevo",
        400
      )
    );
  }
...
exports.updatePassword = catchAsyncErrors(async (req, res, next) => {
  ...
  //Revisamos si la contraseña anterior es igual a la nueva
